...
from verl_patch.workers.code.rollout.rollout_worker import RolloutWorker
from verl_patch.workers.config import (
    FSDPCriticConfig,
    FSDPEngineConfig,
    HFModelConfig,
    RolloutConfig,
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        if self._is_actor:
            ...
            # In multi-turn setup, we should multiply mini batch size by max_turns to keep it as on policy update
            if self.config.rollout.multi_turn.enable:
                self.config.actor.ppo_mini_batch_size *= self.config.rollout.multi_turn.max_user_turns
    ...
    def init_model(self):
        from verl_patch.workers.code.actor.dp_actor import (
            CodeDataParallelPPOActor as DataParallelPPOActor,
        )
        ...
        if self._is_actor:
            ...
            # Create actor - CodeDataParallelPPOActor now handles sum_pi_squared automatically
            # based on config.actor.compute_sum_pi_squared
            ...
            if hasattr(self.config.actor, 'compute_sum_pi_squared') and self.config.actor.compute_sum_pi_squared:
                logger.info("Actor initialized with sum_pi_squared computation enabled")
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        compute_sum_pi_squared = (
            hasattr(self.config.actor, 'compute_sum_pi_squared') and self.config.actor.compute_sum_pi_squared
        )
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                # Actor returns tuple: (log_probs, entropys, sum_pi_squared)
                # sum_pi_squared is None when not requested
                log_probs, entropys, sum_pi_squared = self.actor.compute_log_prob(
                    data=data, calculate_entropy=True, compute_sum_pi_squared=compute_sum_pi_squared
                )

                # Build DataProto with available tensors
                tensors = {"old_log_probs": log_probs, "entropys": entropys}
                if sum_pi_squared is not None:
                    tensors["sum_pi_squared"] = sum_pi_squared
